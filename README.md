...
> Cloud Build トリガーを利用する場合は、ビルドステップの `--file`/`-f` で **リポジトリルートの Dockerfile** を参照するようにしてください（`/workspace/Dockerfile`）。
...
### Cloud Build トリガー用 cloudbuild.yaml
ルートの Dockerfile を確実に参照するため、`cloudbuild.yaml` を同梱しています。ビルドトリガーで
環境変数 `_IMAGE` を適切な Artifact Registry のフルイメージ名に設定し、この設定ファイルを
使ってビルドしてください。
